 ...
 *
 * <pre>./mvnw javafx:run -DmainClass=src.fx.ui.NavigationSmokeTest</pre>
 */
public class NavigationSmokeTest extends Application {
    ...
    public void start(Stage stage) {
        ...
        clicked += fireAll("start page",
                new src.fx.screens.StartPageScreen().getView(), failures);
        ...
        clicked += fireAll("Lernen → lesson",
                new src.fx.screens.learning.LearningSelectionScreen().getView(), failures);
        clicked += fireAll("Minispiele → lesson",
                new src.fx.screens.games.memory.MemorySelectionScreen().getView(), failures);

        // The library lists every word in the selected lesson.
        clicked += fireAll("Bibliothek → word",
                new src.fx.screens.learning.LibraryViewScreen().getView(), failures);

        // The memory game: Start with no input falls back to one pair, then
        // click each card, which covers reveal, match and mismatch.
        Parent memory = new src.fx.screens.games.memory.MemoryMainScreen(
